...
namespace Igloo
{
    ...
    public class SSTGlobalTypes : SubsectionData
    {
        ...
        public override void Read(BinaryReader reader)
        {
            ...
            uint[] typeOffsets = new uint[typeCount];
            TypeNames = new string[typeCount];

            for (int i = 0; i < typeCount; i++)
            {
                typeOffsets[i] = reader.ReadUInt32();
            }

            // For NB09 executables, the type string offset is from the first type record of the sstGlobalTypes subsection
            long typeBaseAddress = reader.BaseStream.Position;

            for (int i = 0; i < typeCount; i++)
            {
                reader.BaseStream.Position = typeBaseAddress + typeOffsets[i];
