...
namespace Igloo
{
    public class ExportSectionReader : ISectionHandler
    {
        ...
        {
            ExportSection cs = new ExportSection { SectionHeader = Header };

            // Read Export directory table
            ExportDirectoryTable exportDirTbl = Misc.FromBinaryReader<ExportDirectoryTable>(r);

            // Read export address table
            //r.BaseStream.Position = PeHeader.OptionalHeader32.ImageBase + exportDirTbl.OrdinalTableRVA;
            List<ExportOrdinalTableEntry> exportAddressTable = new List<ExportOrdinalTableEntry>(exportDirTbl.AddressTableEntries);

            for (int i = exportDirTbl.AddressTableEntries; i > 0; i--)
            {
                uint exportRVA = r.ReadUInt32();
                uint forwarderRVA = r.ReadUInt32();

                ExportOrdinalTableEntry exportOrdinalTableEntry = new ExportOrdinalTableEntry();
                exportOrdinalTableEntry.ExportRVA = exportRVA;
                exportOrdinalTableEntry.ForwarderRVA = forwarderRVA;
                exportAddressTable.Add(exportOrdinalTableEntry);
            }

            // Export Name Pointer Table
            //r.BaseStream.Position = PeHeader.OptionalHeader32.ImageBase + exportDirTbl.NamePointerRVA;
            List<uint> exportNamePointers = new List<uint>(exportDirTbl.NamePointerCount);
            ...
            //r.BaseStream.Position = PeHeader.OptionalHeader32.ImageBase + exportDirTbl.OrdinalTableRVA;
            List<ushort> symbolAddressLookup = new List<ushort>(exportDirTbl.AddressTableEntries);

            // Export name table
            List<string> exportNames = new List<string>(exportDirTbl.NamePointerCount);
